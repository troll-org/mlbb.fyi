...
const PostContent: React.FC<PostContentProp> = ({
  ...
  async function handleLikeDislike(action: string) {
    ...
    const set = await fetch("/explore/api/post-action?action=" + action, {
      method: "POST",
      body: JSON.stringify({
        postId: post.id,
      }),
  ...
  async function handleFavouriteUnfavourite(action: string) {
    ...
    const set = await fetch("/explore/api/favourite-action?action=" + action, {
      method: "POST",
      body: JSON.stringify({
        postId: post.id,
      }),
                      ...
                      >
                        ...
                        {isOpen && (
                          <div className="absolute right-0 mt-2 w-40 origin-top-right ">
                            ...
                            >
                              <button
                                className="block px-4 py-2 text-cloud/80 hover:text-cloud hover:duration-300"
                                onClick={() => {
              ...
              <div>
                <ul role="list" className="flex flex-row items-center gap-1">
                  {post.tags?.map((tag: string) => (
                    <p
                      className="text-xs mt-2 truncate leading-5 text-cloud ease-in-out"
                      key={tag}
                    >{`#${tag}`}</p>
                  ))}
                </ul>
              ...
              <span>
                {expandedable && (
                  <button
                    ...
                    className="font-bold text-cloud/80 transition-all ease-in-out hover:underline hover:duration-300"
                  >
