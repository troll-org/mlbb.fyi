    ...
    <>
      ...
      <div className="mb-4 flex flex-row items-center gap-5 ">
          ...
          {!loading && !like && (
            <button
              onClick={async () => {
                ...
                const set = await fetch("/explore/api/comLike", {
                  method: "POST",
          ...
          {!loading && !dislike && (
            <button
              onClick={async () => {
                ...
                const set = await fetch("/explore/api/comDislike", {
                  method: "POST",
                  body: JSON.stringify(fields),
                });
                const msg = await set.json();
        ...
        {comment.replies && comment.replies.length !== 0 && (
          <div>
            <button
              className="flex cursor-pointer flex-row items-center transition-all ease-in-out hover:text-sea hover:duration-300"
